# #!/usr/bin/env python3
# coding=utf-8
# Mike's Demo - put the LEGO Boost through its paces
# By Andrey Pokhilko & Mike Cook Feb 2019
# Improved by ZXDad for Raspberry Pi 

from time import sleep
import time
from pylgbst import *
from pylgbst import get_connection_bleak
from pylgbst.hub import MoveHub, COLORS
from pylgbst.peripherals import EncodedMotor, TiltSensor, Current, Voltage

def main():
  print("Mr. Vernie all you alright?")
  ...
  try:
    ...
    sleep(2.0)
    print("Level One Diagnostic Complete. I am functioning within normal parameters")
  finally:  
    conn.disconnect()
...
def demo_led_colors(movehub):
    ...
    for colour in range(1,11):
        print("Setting LED color to: %s" % COLORS[colour])
...
"Tilt #%s of %s: roll:%s pitch:%s yaw:%s" % (demo_tilt_sensor_simple.cnt, limit, roll,pitch,yaw))
    ...
    movehub.tilt_sensor.subscribe(callbackTilt, mode=TiltSensor.MODE_3AXIS_ACCEL, granularity=3)
    while demo_tilt_sensor_simple.cnt < limit:
        sleep(1)
...
def demo_color_sensor(movehub):
    ...
    try:
        movehub.vision_sensor.subscribe(callback_color)
    except:
    ...
    movehub.vision_sensor.unsubscribe(callback_color)
